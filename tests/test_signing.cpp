...
template<size_t 𝜅, size_t k, size_t l, size_t d, size_t 𝑢w, size_t 𝑢t, size_t 𝜈w, size_t 𝜈t, size_t rep, size_t 𝜔, size_t sig_len, uint64_t Binf, uint64_t B22>
static void
test_signing(const size_t mlen)
{
  ...
  raccoon::sign<𝜅, k, l, d, 𝑢w, 𝜈w, 𝜈t, rep, 𝜔, sig_len, Binf, B22>(skey, msg, sig);
  const auto is_verified = raccoon::verify<𝜅, k, l, 𝜈w, 𝜈t, 𝜔, sig_len, Binf, B22>(pkey, msg, sig);
...
TEST(RaccoonSign, Raccoon128Signing)
{
  ...
  constexpr size_t sig_len = 11524;
  constexpr uint64_t Binf = 41954689765971ul;
  constexpr uint64_t B22 = 14656575897ul;

  for (size_t mlen = min_mlen; mlen <= max_mlen; mlen++) {
    test_signing<𝜅, k, l, 1, 41, 6, 𝜈w, 𝜈t, 8, 𝜔, sig_len, Binf, B22>(mlen);
    test_signing<𝜅, k, l, 2, 41, 6, 𝜈w, 𝜈t, 4, 𝜔, sig_len, Binf, B22>(mlen);
    test_signing<𝜅, k, l, 4, 41, 6, 𝜈w, 𝜈t, 2, 𝜔, sig_len, Binf, B22>(mlen);
    test_signing<𝜅, k, l, 8, 40, 5, 𝜈w, 𝜈t, 4, 𝜔, sig_len, Binf, B22>(mlen);
    test_signing<𝜅, k, l, 16, 40, 5, 𝜈w, 𝜈t, 2, 𝜔, sig_len, Binf, B22>(mlen);
